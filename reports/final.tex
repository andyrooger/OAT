...
We also record information to identify changes to the tree. When we save or load a file, we also record a SHA-224 hash of the file. This
is used to ensure we are loading the correct tree when we request the file later. We also keep track of a \texttt{modified} flag that tells
us if the tree structure is changed, and an \texttt{augmented} flag that lets us know whether any other information has been added to the tree.
...
Now we can look at an example session:
