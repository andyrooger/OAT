...
path will not ever be followed, it cannot safely remove our dead code.
...
A three examples in \cite{taxobftrans} are
...
three types we will also introduce our own branching on a \texttt{while} loop.
...
We will implement these operations by creating a \texttt{Brancher} class. This will define the following methods:
...
\# The branches we will insert will jump either to our valid program code, or another obfuscated version of our code or even some cleverly disguised garbage. The purpose of doing so being
...
add false branch to dead code
branch and duplicate our code
branch with while?
branch true with our code
insert bug to code in one branch

--------------------------------------------------------------------

Add very basic opaque predicate support.
   i.e. ask user to add a variable to use, condition on it that's always true,
   and an operation that looks complicated but won't change it. Then ask the
   user to find a list of statements where it would be accessible and add it.
