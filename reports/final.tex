...
development of the analysis that requires them.

We can now create a command for oat to extend this functionality to the user. This means that we can experiment and alter markings without
the prior programmatic analysis.

\subsection{Extending Marking Functionality to the User}

\# Develop mark command, don't mention automarker parts, we can add these later
\# Develop automarker
\# Find some way to tie these in and make them sensible while still not having any real marking types
\# Possibly suggest some examples of marking types earlier on
