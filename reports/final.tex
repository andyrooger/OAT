...
\usepackage{listings}
\lstdefinelanguage{obftool}{morekeywords={help,quit,status,parse,explore,format,visualise,reorder,mark,branch}}
...
\begin{abstract}
Not so long ago, a programmer could write their program in a compiled language
such as C. This would then be compiled to machine code for a given architecture,
and the program could be handed out to anyone without worrying about whether
a hacker or reverse-engineer could recover the original source code.

With the increasing popularity of interpreted languages like Java or Python, we
are starting to see a change in the way that program code is distributed. In these
interpreted formats, a distributed program is often so close to the original source code that 
reverse engineering becomes almost effortless and holding onto embedded secrets or avoiding
piracy can be incredibly difficult.

We propose source code control-flow obfuscation as a solution to this problem in Python and
explore the creation of tools to perform this obfuscation as well as to reverse it. We will
see the additional difficulties faced by when using such a flexible language and will arrive at
a basic but functional obfuscator named OAT.

\end{abstract}

\section*{Acknowledgements}

I would like to thank my supervisor, Herbert Wiklicky, for his input and guidance during the course of this project, as well Chris Hankin for his
constructive feedback. I would also like to thank Alex Lamaison for helping me to learn how to battle the Python.

Finally I would like to thank my family and friends for their support, and most of all, my wonderful fianc\'ee, Hayley Hannam, for
putting up with me ignoring her for work and for keeping me almost sane for the last few years.
...
By providing the \texttt{\_\_str\_\_} method, we allow conversion from node to string using \texttt{str(node)}. This means we can program in a much more
descriptive manner.
...
We may illustrate the differences between our two solid implementations of the \texttt{SourceWriter} by presenting the following sample program passed
...
If we were to parallelise our code then we could do this easily by using the \texttt{multiprocessing} standard library. This makes it
...
the \texttt{\_markings} dictionary. \texttt{is\_marked} will tell us if the name was successfully located, and \texttt{get\_mark} will return either the
...
\begin{description}
  ...
  \item[``add\_reads''/``add\_writes'': set of children containing names] \hfill \\
...
The constructor only passes its arguments through to the constructors of \texttt{BasicReorderer} and \texttt{ReorderChecker}. \texttt{precond} is
...
Now we ensure that the first two of our rules are covered by calling \texttt{\_split\_by\_visibility} to take out any visible statements from our permutation.
...
This concludes our design for the \texttt{SingleReorderer}. The most important part of this discussion is to see that the three functions
...
We now have four different valuers to encourage the recovery of a simple or natural program order. These valuers are \texttt{WriteRangeValuer}, \texttt{WriteUseValuer},
...
The way we do this is to replace the \texttt{SingleReorderer} that \texttt{Reorderer} would use with a modified version. In this version we override
...
\texttt{True} or \texttt{False}. The predicate should be designed so that \texttt{bool(predicate)} evaluates to \texttt{value} at any point where \texttt{name} has
...
We also allow iteration over the \texttt{predicates} object and define all of the necessary methods to allow counting of the number of predicates using
...
was mentioned in the discussion preceding this decision, a GUI would provide a more natural interface to the user.
...
Now we compare \texttt{RandomReorderer} to \texttt{Reorderer}. We can reasonably expect that \texttt{RandomReorderer} will be slower because it adds extra storage and random
...
\lstinputlisting[language=obftool,basicstyle=\small,breaklines=true]{../snippets/usersession.log.1}
\centering
\scalebox{0.33}
{\includegraphics{../images/usersession.eps.2}}
\lstinputlisting[language=obftool,basicstyle=\small,breaklines=true,firstnumber=last]{../snippets/usersession.log.3}
\pagebreak
\lstinputlisting[language=obftool,basicstyle=\small,breaklines=true,firstnumber=last]{../snippets/usersession.log.4}
\scalebox{0.33}
{\includegraphics{../images/usersession.eps.5}}
\lstinputlisting[language=obftool,basicstyle=\small,breaklines=true,firstnumber=last]{../snippets/usersession.log.6}
