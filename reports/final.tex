...
\title{Obfuscating Python 3000 - Final Report}
...
Hopefully this will make the source writer easily extendable to create pretty printers or minifiers\footnote{Minification of a program involves shrinking it's file size by removing as many unnecessary characters as possible. It is often used for interpreted languages such as JavaScript that need to be lightweight for transmission.} and so can allow for a degree of layout
obfuscation if we choose the expand our implementation later. As an example, the following is a sample program passed through the current implementation of BasicWriter. We can see the source code
...
This unintentional layout obfuscation should be fixed later by creating a pretty printer for the code. All of these writers will be held in the writer package.
