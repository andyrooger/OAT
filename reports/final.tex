...
At the beginning of this project the choice was made to implement our primary interface on the command line, primarily for the speed of development. As mentioned
in the discussion, a GUI would provide a more natural interface to the user where the only downside would be the extra time spent in the implementation.

As we have progressed through the development of OAT there are many points we have passed that could have been greatly improved by this graphical interaction.
...
click on a function definition within the tree and continue our analysis in the function body. There is no natural way to do this in the current command line
tool.
...
highlighting our node's source code so that the user can view it in context. This pane would be available throughout the entire programs operation.
...
This is the first component we will develop for GOAT. Our intentions in designing this component are to create a reuseable component that can be used by any
part of the interaface that needs to allow a user to browse and select nodes. We should be able to set up a node browser just by passing a root node to the
...
\begin{description}
\item[\texttt{\_\_init\_\_(parent, fulltree, currenttree, select\_handler, **other)}] \hfill \\
\texttt{parent} and \texttt{other} specify the parent window and any other \texttt{ttk.Treeview} constructor arguments to allow some flexibility in the component.
More interestingly we specify the AST to display with \texttt{fulltree} and the currently selected node with \texttt{currenttree}.

After setting up two detail columns to display each node's type and value (if available), we pass both nodes to \texttt{\_fill\_tree} to populate our displayed
tree. Two tags are set up to colour the selected node and any empty nodes. This allows the user to quickly see the selected tree, and any empty nodes are shaded
out to emphasise the fact that they are not needed.
...
identities in the displayed tree, to node objects allowing \texttt{\_selected} to convert back. We also tag the appropriate node as well as extracting
information from the notes to display in the details column.
\end{description}

We will also create a \texttt{ScrolledASTTreeview} that contains \texttt{ASTTreeview}, allowing the large widget to scroll by adding scrollbars around
the outside.
...
We will implement this in the class \texttt{MarkPane}, whose constructor takes just a node whose markings to display. Each type of marking specified on the
node is given a row for display and the actual mark is converted using a very primitive algorithm into text.
...
The first class we will create is \texttt{CodeBox}, a child of \texttt{tkinter.Text}. This class has a very simple constructor, taking no arguments and
defining two tags, one for highlighting the current node (for example the one highlighted by the \textbf{explore} command), and one for highlighting
a selected node.

The only other function defined on this class is \texttt{fill} which is given a writer class and a node to generate the source code from, and two nodes to
highlight with the tags we defined in the constructor. In this function we reach a difficulty when we come to tag the code we wish to highlight. The design
of the underlying \texttt{tkinter.Text} class requires that to highlight a section of the source text we will need to either tag the whole section as we
insert it into the text box at once, or we will need to be able to mark both ends of the highlighted section as we write it.
...
using the \texttt{tagging\_writer} function to dynamically generate a subclass our our writer class. This will overwrite the \texttt{\_write} function
to capture every node before and after it is written, generating the start and end tags. We will also generate a file-like object that allows all writes
the be converted to inserts into the text box.
...
A final class will now be created called \texttt{CodeDisplay}. This adds a radio selection to decide on the source writer, allowing the user to change
the writer class without closing the window.

\subsection{Growing GOAT from OAT}
...
line interface, OAT. For the initial three components we have created we will introduce the command \textbf{visualise}. This will combine all three
components into a single window that allows a user to inspect the current state of their AST and its markings in the graphical interface, but will
not allow them to change anything.

By creating a command to display this we are also able to test any of the graphical components ready to incorporate them into the full version of GOAT
in the future. For an example of the use of this graphical interface, please refer to the user session in the appendix.
